      ...
      <section style={{ padding: '100px 80px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ color: '#888', lineHeight: 1.8 }}>
            ANDO는 구의정원 작가의 작업실로, 테라리움과 비바리움 작품이 탄생하는 곳입니다.
            이곳에서는 원데이 클래스를 통해 직접 작은 정원을 만들어보실 수 있습니다.
          </p>
        </div>
      </section>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '24px', maxWidth: '800px', margin: '0 auto' }}>
              {classes.map((item, index) => (
                <div key={index} style={{ padding: '32px', border: '1px solid #333', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <p style={{ fontSize: '18px', fontWeight: 300 }}>{item.price}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ marginBottom: '80px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '24px', maxWidth: '800px', margin: '0 auto' }}>
              {customOrders.map((item, index) => (
                <div key={index} style={{ padding: '32px', border: '1px solid #333', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <h3 style={{ fontSize: '16px', fontWeight: 300, letterSpacing: '0.05em' }}>{item.title}</h3>
                  <p style={{ fontSize: '18px', fontWeight: 300, color: '#888' }}>{item.price}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '24px' }}>
            {studioImages.map((image) => (
              ...
              >
                <p style={{ color: '#666', fontSize: '14px' }}>{image.alt}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '100px 80px' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ color: '#888', lineHeight: 1.8, marginBottom: '32px' }}>
            작업실 방문은 예약제로 운영됩니다.<br />
            원데이 클래스 예약 및 작업실 방문 문의는 인스타그램 DM으로 부탁드립니다.
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
              <p style={{ fontSize: '12px', color: '#888', marginBottom: '8px' }}>Instagram DM</p>
              <a href="https://www.instagram.com/gu.garden/" target="_blank" rel="noopener noreferrer" style={{ fontSize: '16px', color: '#fff', textDecoration: 'none' }}>
                @gu.garden
              </a>
            </div>
            <div>
              <p style={{ fontSize: '12px', color: '#888', marginBottom: '8px' }}>Tel</p>
              <a href="tel:0507-1424-7573" style={{ fontSize: '16px', color: '#fff', textDecoration: 'none' }}>
                0507-1424-7573
              </a>
            </div>
          </div>
